...
export function newsContext(article = {}) {
  return [...new Set([article.description, article.content].map(cleanNewsExcerpt).filter(Boolean))].join(' ');
}
...
export async function answerNewsQuestion({ currentAffairs, question, recentMessages = [], provider, model, generate = generateResponse }) {
  ...
  const context = JSON.stringify({ headline: article.title, excerpt: newsContext(article), source: article.source, publishedAt: article.publishedAt });
  try {
    ...
    const source = [article.title, newsContext(article)].join(' ');
    if (typeof parsed.answer === 'string' && parsed.answer.trim() && parsed.answer.length <= 650 &&
        typeof parsed.evidence === 'string' && parsed.evidence.trim().length >= 12 && source.includes(parsed.evidence.trim())) return parsed.answer.trim();
  } catch {
