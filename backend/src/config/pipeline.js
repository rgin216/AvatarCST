...
//   openai-fast-scripted   -> OpenAI transcription -> deterministic script -> streaming OpenAI TTS -> audio energy lipsync
...
export const SESSION_PIPELINE_MODES = [
  'free',
  'openai-scripted',
  'openai-scripted-energy',
...
export const usesOpenAITextPipeline = (mode) =>
  isOpenAIScriptedPipeline(mode)
  || isOpenAIScriptedEnergyPipeline(mode)
  || isOpenAIFastScriptedPipeline(mode)
  || isOpenAIAudioPipeline(mode);
