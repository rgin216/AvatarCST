...
- Landing flow with eight test sessions:
...
- Slide-backed session UI using exported slide images under `frontend/public/slides/session1/` through `session8/`.
...
- `backend/context/vCST_Session1_AI_Script.md` through `vCST_Session8_AI_Script.md` hold fuller source script text and adaptation guidance. Each markdown file has one `---`-separated section per executable step, in order, after a preamble section.
- `backend/context/vCST_Initial_Prompt.md` defines Aria's tone and CST facilitation principles.

Slide images live in:

- `frontend/public/slides/session1/` through `frontend/public/slides/session8/`
